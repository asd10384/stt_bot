...
import { Message, EmbedBuilder, ApplicationCommandOptionType, ChatInputApplicationCommandData, CommandInteraction, Guild, TextChannel } from "discord.js";
...
export default class implements Command {
  ...
  async messageRun(message: Message, args: string[]) {
    ...
    if (!guildDB) return (message.channel as TextChannel).send({ embeds: [ client.mkembed({
      title: `데이터베이스오류`,
      description: "다시시도해주세요.",
      color: "DarkRed"
    }) ] }).then(m => client.msgdelete(m, 1));
    if (args[0] === "목록") return (message.channel as TextChannel).send({ embeds: [ this.list(guildDB) ] }).then(m => client.msgdelete(m, 4));
    if (args[0] === "추가") {
      if (args[1]) {
        const role = message.guild?.roles.cache.get(args[1]);
        if (role) return (message.channel as TextChannel).send({ embeds: [ await this.add(message.guild!, guildDB, role.id) ] }).then(m => client.msgdelete(m, 2));
      }
    ...
    if (args[0] === "제거") {
      if (args[1]) {
        const role = message.guild?.roles.cache.get(args[1]);
        if (role) return (message.channel as TextChannel).send({ embeds: [ await this.remove(message.guild!, guildDB, role.id) ] }).then(m => client.msgdelete(m, 2));
      }
